...
public class IntList {
    ...
    private static IntList insertBack(IntList oldList, int lastItem) {
        ...
        oldList.tail = new IntList(lastItem, null);
        return newList;
    }

    /**
     * A helper method for recursive reverse method.
     */
    private static IntList reverseRecursiveHelper(IntList A) {
        /* If zero or one item, then return the IntList. */
        if (A == null || A.tail == null) {
            return A;
        }
        /* At least two items. Reverse Everything except the first int. */
        IntList reversed = reverseRecursiveHelper(A.tail);
        /* Put the front int to the back of the new reversed IntList. */
        reversed = insertBack(reversed, A.head);
        return reversed;
    }
     ...
     * as an input, return null. Using recursion with
     * the help of the reverseRecursiveHelper().
     */
